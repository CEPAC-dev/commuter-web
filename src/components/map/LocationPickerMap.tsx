...
import { searchAddress, getPlaceDetails, reverseGeocode, formatDisplayName } from '@/lib/nominatim';
...
export default function LocationPickerMap({ lat, lng, name, onChange, error }: LocationPickerMapProps) {
  ...
  return (
    <div>
      ...
      <div style={{ position: 'relative', marginBottom: 8 }}>
        <div
          ...
          onFocusCapture={(e) => {
            ...
            (e.currentTarget as HTMLElement).style.boxShadow = '0 0 0 3px rgba(0,194,168,0.15)';
          }}
          onBlurCapture={(e) => {
            (e.currentTarget as HTMLElement).style.borderColor = error ? '#E74C3C' : '#D1D5DB';
            (e.currentTarget as HTMLElement).style.boxShadow = '0 2px 8px rgba(0,0,0,0.06)';
          }}
        >
          {searching
            ? <Loader2 size={16} className="animate-spin" style={{ color: '#00C2A8', flexShrink: 0 }} />
          ...
          <input
            ...
            style={{
              ...
              fontSize: 13, color: '#0B1E3D', fontFamily: 'inherit',
            }}
          />
          {query && (
            <button
              type="button"
              onClick={() => { setQuery(''); setResults([]); setShowDrop(false); onChange('', '', ''); }}
      ...
      <div style={{
        ...
        {!isLoaded ? (
          <div style={{ height: 280, display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#F0EDE6', color: '#5A6A7A', fontSize: 13, gap: 8 }}>
            <Loader2 size={18} className="animate-spin" style={{ color: '#00C2A8' }} />
            Loading map…
          </div>
        ) : (
          <GoogleMap
          ...
          >
            {markerPos && (() => {
              ...
              const zf   = Math.max(0.5, Math.min(1.2, Math.pow(2, (zoom - 14) / 5)));
        ...
        {isLoaded && (
          <button
            ...
            onMouseEnter={(e) => { if (!locating) { e.currentTarget.style.boxShadow = '0 4px 14px rgba(0,194,168,0.3)'; e.currentTarget.style.transform = 'scale(1.05)'; } }}
            onMouseLeave={(e) => { e.currentTarget.style.boxShadow = '0 2px 8px rgba(11,30,61,0.15)'; e.currentTarget.style.transform = 'scale(1)'; }}
          >
            {locating
        ...
        {!markerPos && isLoaded && (
          <div style={{
            ...
            background: 'rgba(11,30,61,0.8)', color: '#fff',
      ...
      {error && <p style={{ marginTop: 6, fontSize: 12, color: '#E74C3C' }}>{error}</p>}
    </div>
  );
}
